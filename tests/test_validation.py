...
def test_missing_total_requires_review() -> None:
    report = validate_receipt(
        base_receipt(total=None),
        today=date(2026, 9, 25),
    )
    assert not report.auto_accept
    assert "total_missing" in {
...
def test_receipt_arithmetic_mismatch_requires_review() -> None:
    report = validate_receipt(
        base_receipt(total="11.60"),
        today=date(2026, 9, 25),
    )
    assert not report.auto_accept
    assert "receipt_arithmetic_mismatch" in {
        issue.code for issue in report.blockers
    }
...
def test_line_item_mismatch_is_warning_only() -> None:
    receipt = base_receipt(
        line_items=[
            LineItem(
                description_raw="ITEM A",
                amount=Decimal("4.00"),
            ),
            LineItem(
                description_raw="ITEM B",
                amount=Decimal("4.00"),
            ),
        ]
    )
    ...
    assert report.auto_accept
    assert "line_item_sum_mismatch" in {
